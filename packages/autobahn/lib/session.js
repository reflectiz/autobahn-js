...
var Session = function (
  ...
  self._process_SUBSCRIBE_ERROR = function (msg) {
    ...
    if (request in self._subscribe_reqs) {
      var details = msg[3];
      var error = self.preError(
        new AutobahnRemoteError("Error while subscribing.", {
          name: msg[4],
          args: msg[5],
          kwargs: msg[6],
        })
  ...
  self._process_UNSUBSCRIBE_ERROR = function (msg) {
    ...
    if (request in self._unsubscribe_reqs) {
      var details = msg[3];
      var error = self.preError(
        new AutobahnRemoteError("Error while unsubscribing.", {
          name: msg[4],
          args: msg[5],
          kwargs: msg[6],
        })
  ...
  self._process_PUBLISH_ERROR = function (msg) {
    ...
    if (request in self._publish_reqs) {
      var details = msg[3];
      var error = self.preError(
        new AutobahnRemoteError("Error while publishing.", {
          name: msg[4],
          args: msg[5],
          kwargs: msg[6],
        })
  ...
  self._process_REGISTER_ERROR = function (msg) {
    ...
    if (request in self._register_reqs) {
      var details = msg[3];
      var error = self.preError(
        new AutobahnRemoteError("Error while registering operation.", {
          name: msg[4],
          args: msg[5],
          kwargs: msg[6],
        })
  ...
  self._process_UNREGISTER_ERROR = function (msg) {
    ...
    if (request in self._unregister_reqs) {
      var details = msg[3];
      var error = self.preError(
        new AutobahnRemoteError("Error while unregistering.", {
          name: msg[4],
          args: msg[5],
          kwargs: msg[6],
        })
  ...
  self._process_CALL_ERROR = function (msg) {
    ...
    if (request in self._call_reqs) {
      var details = msg[3];
      var error = self.preError(
        new AutobahnRemoteError("Error during RPC call.", {
          name: msg[4],
          args: msg[5],
          kwargs: msg[6],
        })
